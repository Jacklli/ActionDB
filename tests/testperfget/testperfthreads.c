...
char *errmsg = NULL;
    char command_get[50] = "";
    char command_get_key[20] = "";
    char command_get_val[20] = "";
    char readBuf[20];
    char *ptr = NULL;
    struct hostent *host;
...
//printf("in thread %lu, arg1 is:%d, arg2 is:%d\n", pthread_self(), ((args *)arg)->arg1, ((args *)arg)->arg2);
    for(loopcnt = ((args *)arg)->arg1; loopcnt < ((args *)arg)->arg2; loopcnt++) {
        strcpy(command_get,"get\ta");
        sprintf(command_get_key,"%d",loopcnt);   //fill key with loopcnt
        strcat(command_get,command_get_key);     //combine commend_get and command_get_key
        ptr = makeCommand(command_get);
//        printf("%s\n", ptr);
...
printf("\n\n");
printf("start get test with 1 threads.\n");
